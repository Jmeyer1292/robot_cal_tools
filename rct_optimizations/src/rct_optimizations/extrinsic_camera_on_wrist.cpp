...
{
  if (params.image_observations.size() != params.wrist_poses.size())
    throw OptimizationException("image_observations.size() != wrist_poses.size()");
  ...
  try
  {
    result.covariance_wrist_to_camera = rct_optimizations::computePoseCovariance(problem, internal_camera_to_wrist);
  }
  catch (CovarianceException &e)
  {
    std::cout << "Failed to calculate covariance for camera_to_wrist transform: " << e.what() << std::endl;
  }

  try
  {
    result.covariance_base_to_target = rct_optimizations::computePoseCovariance(problem, internal_base_to_target);
  }
  catch (CovarianceException &e)
  {
    std::cout << "Failed to calculate covariance for base_to_target transform: " << e.what() << std::endl;
  }

  try
  {
    result.covariance_w2c_vs_b2t = rct_optimizations::computePose2PoseCovariance(problem, internal_camera_to_wrist, internal_base_to_target);
  }
  catch (CovarianceException &e)
  {
    std::cout << "Failed to calculate off-diagonal covariance between camera_to_wrist transform and base_to_target transform: " << e.what() << std::endl;
  }
