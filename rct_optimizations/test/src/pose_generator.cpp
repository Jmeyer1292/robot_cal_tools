...
rct_optimizations::test::genHemispherePose(Eigen::Isometry3d target_pose, double r, int theta_cnt, int phi_cnt)
{
  ...
  for (long theta_it = 0; theta_it < theta_range.size(); ++theta_it)
  {
    ...
    for (long phi_it = 0; phi_it < phi_range.size(); ++phi_it)
    {
      ...
      Eigen::Isometry3d camera_oriented =
          rct_optimizations::test::lookAt(camera.translation(), target_pose.translation(), Eigen::Vector3d(1, 0, 0));
...
std::vector<Eigen::Isometry3d>
rct_optimizations::test::genConicalPose(Eigen::Isometry3d target_pose, int observations, double r, double h)
{
  std::vector<Eigen::Isometry3d> camera_positions;
  ...
  for (int i = 0; i < observations; ++i)
  {
    ...
    Eigen::Isometry3d camera_oriented =
        rct_optimizations::test::lookAt(camera_pose.translation(), target_pose.translation(), Eigen::Vector3d(1, 0, 0));
...
std::vector<Eigen::Isometry3d>
rct_optimizations::test::genGridPose(Eigen::Isometry3d target_pose, int grid_side, double spacing, double h)
{
  // Generates positions in target frame; need to convert to world frame
  std::vector<Eigen::Isometry3d> camera_positions;
  ...
  for (int i = -1 * (grid_side - 1) / 2; i <= (grid_side - 1) / 2; ++i)
  {
    for (double j = -1 * (grid_side - 1) / 2; j <= (grid_side - 1) / 2; ++j)
    {
      ...
      Eigen::Isometry3d camera_oriented = rct_optimizations::test::lookAt(
          camera_pose.translation(), target_pose.translation(), Eigen::Vector3d(1, 0, 0));
